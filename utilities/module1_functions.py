...
def get_frontier_orbital_energies(outfile, heteroatom) -> list:
    ...
    # the homo-3 to homo-9 energies will be useful for the CF3 radical
    lumo_plus_1_energy = sum(moenergies[:, n_lumo+1])/2 # [hartree]
    lumo_plus_2_energy = sum(moenergies[:, n_lumo+1])/2 # [hartree]
   
    homo_minus_1_energy = sum(moenergies[:, n_homo-1])/2 # [hartree]
    homo_minus_2_energy = sum(moenergies[:, n_homo-2])/2 # [hartree]

    if heteroatom == 'F':
        homo_minus_3_energy = sum(moenergies[:, n_homo-3])/2 # [hartree]
        homo_minus_4_energy = sum(moenergies[:, n_homo-4])/2 # [hartree]
        homo_minus_5_energy = sum(moenergies[:, n_homo-5])/2 # [hartree]

        homo_minus_6_energy = sum(moenergies[:, n_homo-6])/2 # [hartree]
        homo_minus_7_energy = sum(moenergies[:, n_homo-7])/2 # [hartree]
        homo_minus_8_energy = sum(moenergies[:, n_homo-8])/2 # [hartree]

        homo_minus_9_energy = sum(moenergies[:, n_homo-9])/2 # [hartree]
        homo_minus_10_energy = sum(moenergies[:, n_homo-10])/2 # [hartree]
        homo_minus_11_energy = sum(moenergies[:, n_homo-11])/2 # [hartree]

        homo_minus_12_energy = sum(moenergies[:, n_homo-12])/2 # [hartree]
        homo_minus_13_energy = sum(moenergies[:, n_homo-13])/2 # [hartree]
        homo_minus_14_energy = sum(moenergies[:, n_homo-14])/2 # [hartree]

        homo_minus_15_energy = sum(moenergies[:, n_homo-15])/2 # [hartree]


    elif heteroatom == 'H':
        homo_minus_3_energy = sum(moenergies[:, n_homo-3])/2 # [hartree]
        homo_minus_4_energy = 0
        homo_minus_5_energy = 0

        homo_minus_6_energy = 0
        homo_minus_7_energy = 0
        homo_minus_8_energy = 0

        homo_minus_9_energy = 0
        homo_minus_10_energy = 0
        homo_minus_11_energy = 0
    

        homo_minus_12_energy = 0
        homo_minus_13_energy = 0
        homo_minus_14_energy = 0

        homo_minus_15_energy = 0

    datalist = [homo_energy, somo_a_energy, somo_b_energy, lumo_energy, lumo_plus_1_energy, lumo_plus_2_energy, \
                homo_minus_1_energy, homo_minus_2_energy, homo_minus_3_energy, homo_minus_4_energy, homo_minus_5_energy, \
                homo_minus_6_energy, homo_minus_7_energy, homo_minus_8_energy, homo_minus_9_energy, homo_minus_10_energy, \
                homo_minus_11_energy, homo_minus_12_energy, homo_minus_13_energy, homo_minus_14_energy, homo_minus_15_energy]
...
def parse_outfile(outfile_path : str) -> "tuple[list, list]":
    '''
    documentation best practices here
    '''
